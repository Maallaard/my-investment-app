...
function getTradeLabel(type, quantity, summary) {
  ...
  if (type === "buy") {
    if (summary.holdingQty === 0) return { text: "신규 매수", color: COLORS.blue, bg: "#EBF3FE" }
    return { text: "물타기", color: COLORS.orange, bg: "#FFF3E9" }
  } else {
    if (qty >= summary.holdingQty) return { text: "전량 매도", color: COLORS.red, bg: "#FEF0F1" }
...
function getStatusBadge(totalInvested, realizedProfit, unrealizedProfit) {
  ...
  if (realizedProfit >= totalInvested) return { text: "원금 회수 완료", color: COLORS.green, bg: "#E6FAF6" }
  if (totalValue >= 0) return { text: "수익 중", color: COLORS.blue, bg: "#EBF3FE" }
  if (totalValue < 0) return { text: "원금 손실 중", color: COLORS.red, bg: "#FEF0F1" }
...
useEffect(() => {
  ...
  function handleSubmit() {
    ...
    setForm({ ...form, quantity: "", price: "", memo: "" })
  }
  ...
  const previewProfit = form.type === "sell" && form.price && form.quantity && selectedSummary ? (Number(form.price) - selectedSummary.avgPrice) * Number(form.quantity) : null
  ...
  const totalUnrealized = allSummaries.reduce((sum, s) => {
    ...
    return sum + (s.market === "US" ? unrealized * exchangeRate : unrealized)
  }, 0)
  ...
  sellTrades.forEach(t => {
    ...
    monthlyMap[key] = (monthlyMap[key] || 0) + (t.price - avg) * t.quantity
  })
  ...
  const inputStyle = (field) => ({
    ...
    border: errors[field] ? `1.5px solid ${COLORS.red}` : `1px solid ${COLORS.border}`,
    marginBottom: errors[field] ? "4px" : "10px", background: "white", color: COLORS.text, boxSizing: "border-box"
  })

  function ErrorMsg({ field }) {
    if (!errors[field]) return null
    return <div style={{ fontSize: "12px", color: COLORS.red, marginBottom: "10px" }}>⚠ {errors[field]}</div>
  }
...
async function handlePDFImport(e) {
  ...
  try {
    ...
    if (parsed.length === 0) {
      setImportResult({ success: false, message: "거래 내역을 찾지 못했어요. PDF 형식을 확인해주세요." })
      return
    }
    const newStocks = []
    parsed.forEach(t => {
      if (!stockList.find(s => s.name === t.name)) {
        if (!newStocks.find(s => s.name === t.name)) {
          newStocks.push({ name: t.name, market: t.currency })
        }
      }
    })
    if (newStocks.length > 0) setStockList(prev => [...prev, ...newStocks])
    const newTrades = parsed.map(t => ({
  ...
  return (
    <div style={{ maxWidth: "480px", margin: "0 auto", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", background: "#F8F9FA", minHeight: "100vh", paddingBottom: "80px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "12px" }}>
              {[
                ...
                <div key={label} style={{ background: "white", borderRadius: "14px", padding: "16px", textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: "17px", fontWeight: "700", color: value >= 0 ? COLORS.green : COLORS.red }}>
              ...
              <div style={{ background: "white", borderRadius: "14px", padding: "16px", textAlign: "center" }}>
                <div style={{ fontSize: "12px", color: COLORS.textSub, marginBottom: "6px" }}>원금 회수율</div>
                <div style={{ fontSize: "17px", fontWeight: "700", color: recoveryRate >= 100 ? COLORS.green : COLORS.orange }}>{recoveryRate.toFixed(1)}%</div>
                <div style={{ marginTop: "8px", height: "4px", background: COLORS.lightGray, borderRadius: "2px" }}>
                  <div style={{ height: "100%", borderRadius: "2px", background: recoveryRate >= 100 ? COLORS.green : COLORS.blue, width: `${Math.min(100, recoveryRate)}%`, transition: "width 0.3s" }} />
                </div>
              </div>
            </div>
                ...
                <div key={s.name} style={{ background: "white", borderRadius: "14px", padding: "16px", marginBottom: "10px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      ...
                      <span style={{ fontSize: "11px", padding: "2px 7px", borderRadius: "20px", background: isStockUS ? "#E6FAF6" : "#EBF3FE", color: isStockUS ? COLORS.green : COLORS.blue, fontWeight: "500" }}>
                        {isStockUS ? "US" : "KR"}
                      </span>
                    </div>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                    ...
                    <span style={{ fontSize: "14px", fontWeight: "700", color: s.realizedProfit >= 0 ? COLORS.green : COLORS.red }}>
                  ...
                  {s.holdingQty > 0 && bepDiff !== null && (
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px", padding: "8px 10px", background: bepDiff >= 0 ? "#E6FAF6" : "#FEF0F1", borderRadius: "8px" }}>
                      <span style={{ fontSize: "12px", color: COLORS.textSub }}>손익분기점(BEP)</span>
                      <span style={{ fontSize: "13px", fontWeight: "700", color: bepDiff >= 0 ? COLORS.green : COLORS.red }}>
                  ...
                  {s.holdingQty > 0 && (
                    <div style={{ display: "flex", gap: "8px", alignItems: "center", paddingTop: "10px", borderTop: `1px solid ${COLORS.border}`, marginBottom: "8px" }}>
                      ...
                      />
                      {s.currentPrice > 0 && (
                        <span style={{ fontSize: "13px", fontWeight: "700", color: unrealized >= 0 ? COLORS.green : COLORS.red, flexShrink: 0 }}>
                  ...
                  {targetProgress !== null && s.currentPrice > 0 && targetPrices[s.name] > s.avgPrice && (
                    <div style={{ marginBottom: "8px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                        <span style={{ fontSize: "11px", color: COLORS.textSub }}>목표가까지</span>
                        <span style={{ fontSize: "11px", fontWeight: "600", color: COLORS.blue }}>{Math.max(0, targetProgress).toFixed(0)}%</span>
                      </div>
                      <div style={{ height: "5px", background: COLORS.lightGray, borderRadius: "3px", overflow: "hidden" }}>
                        <div style={{ height: "100%", borderRadius: "3px", background: COLORS.blue, width: `${Math.max(0, Math.min(100, targetProgress))}%`, transition: "width 0.3s" }} />
                      </div>
                    </div>
                  )}
                    ...
                    <div style={{ marginTop: "12px", paddingTop: "12px", borderTop: `1px solid ${COLORS.border}` }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginBottom: "12px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                          ...
                          <span style={{ fontSize: "12px", fontWeight: "600", color: COLORS.red }}>-{Math.round(totalIn).toLocaleString()}{isStockUS ? "$" : "원"}</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                          <span style={{ fontSize: "12px", color: COLORS.textSub }}>총 회수금액</span>
                          <span style={{ fontSize: "12px", fontWeight: "600", color: COLORS.green }}>+{Math.round(totalOut).toLocaleString()}{isStockUS ? "$" : "원"}</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                        ...
                        : buyHistory.map((h, i) => (
                          <div key={i} style={{ display: "flex", gap: "10px", marginBottom: "8px", alignItems: "flex-start" }}>
                            <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                              <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: i === 0 ? COLORS.blue : COLORS.orange, marginTop: "4px" }} />
                        ...
                        <div style={{ marginTop: "10px", paddingTop: "10px", borderTop: `1px solid ${COLORS.border}` }}>
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
                            <div style={{ display: "flex", justifyContent: "space-between" }}>
                              ...
                              <span style={{ fontSize: "12px", fontWeight: "600", color: unrealizedRaw >= 0 ? COLORS.green : COLORS.red }}>
                                {unrealizedRaw >= 0 ? "+" : ""}{unrealizedRaw.toFixed(2)}$
                              </span>
                            </div>
                            <div style={{ display: "flex", justifyContent: "space-between" }}>
                              <span style={{ fontSize: "12px", color: COLORS.textSub }}>원화 환산 (현재 환율)</span>
                              <span style={{ fontSize: "12px", fontWeight: "600", color: unrealized >= 0 ? COLORS.green : COLORS.red }}>
                                {unrealized >= 0 ? "+" : ""}{Math.round(unrealized).toLocaleString()}원
                              </span>
                            </div>
                            <div style={{ display: "flex", justifyContent: "space-between" }}>
        ...
        {tab === "trade" && (
          <div>
            <div style={{ background: "white", borderRadius: "16px", padding: "20px" }}>
              <div style={{ fontSize: "15px", fontWeight: "700", color: COLORS.text, marginBottom: "16px" }}>거래 입력</div>
              ...
              <select value={form.name} onChange={e => { setForm({ ...form, name: e.target.value, quantity: "", price: "" }); setErrors({ ...errors, name: null }) }} style={{ ...inputStyle("name"), background: "white" }}>
                ...
                {stockList.map(s => <option key={s.name} value={s.name}>{s.name} ({s.market === "KR" ? "한국" : "미국"})</option>)}
              </select>
              <ErrorMsg field="name" />
              {selectedSummary && selectedSummary.holdingQty > 0 && (
                <div style={{ background: "#EBF3FE", borderRadius: "10px", padding: "10px 14px", marginBottom: "10px", fontSize: "13px", color: COLORS.blue, fontWeight: "500" }}>
                  현재 보유 {selectedSummary.holdingQty}주 · 평단 {Math.round(selectedSummary.avgPrice).toLocaleString()}{isUS ? "$" : "원"}
                </div>
              )}
              <div style={{ display: "flex", gap: "8px", marginBottom: "10px" }}>
                <button onClick={() => { setForm({ ...form, type: "buy", quantity: "", price: "" }); setErrors({}) }} style={{ flex: 1, padding: "12px", borderRadius: "10px", border: "none", background: form.type === "buy" ? COLORS.blue : COLORS.lightGray, color: form.type === "buy" ? "white" : COLORS.textSub, fontWeight: "700", fontSize: "14px", cursor: "pointer" }}>매수</button>
                <button onClick={() => { setForm({ ...form, type: "sell", quantity: "", price: "" }); setErrors({}) }} style={{ flex: 1, padding: "12px", borderRadius: "10px", border: "none", background: form.type === "sell" ? COLORS.red : COLORS.lightGray, color: form.type === "sell" ? "white" : COLORS.textSub, fontWeight: "700", fontSize: "14px", cursor: "pointer" }}>매도</button>
              </div>
              ...
              <input type="text" inputMode="decimal" placeholder={isUS ? "수량 (소수점 가능, 예: 0.5)" : "수량"} value={form.quantity}
                onChange={e => { const raw = e.target.value; if (isUS ? /^\d*\.?\d*$/.test(raw) : /^\d*$/.test(raw)) { setForm({ ...form, quantity: raw }); setErrors({ ...errors, quantity: null }) } }}
              ...
              {form.type === "sell" && selectedSummary && selectedSummary.holdingQty > 0 && !form.quantity && (
                <button onClick={() => setForm({ ...form, quantity: String(selectedSummary.holdingQty) })} style={{ width: "100%", padding: "10px", borderRadius: "10px", border: `1px solid ${COLORS.red}`, background: "transparent", color: COLORS.red, fontSize: "13px", fontWeight: "600", marginBottom: "10px", cursor: "pointer" }}>
              ...
              <input type="text" inputMode="decimal" placeholder={isUS ? "달러 단가" : "원화 단가"}
                ...
                onChange={e => { const raw = e.target.value.replace(/,/g, ""); if (isUS ? /^\d*\.?\d*$/.test(raw) : /^\d*$/.test(raw)) { setForm({ ...form, price: raw }); setErrors({ ...errors, price: null }) } }}
              ...
              {tradeLabel && form.quantity && form.price && (
                <div style={{ borderRadius: "12px", padding: "12px 14px", marginBottom: "10px", background: tradeLabel.bg }}>
                  <div style={{ fontSize: "13px", fontWeight: "700", color: tradeLabel.color, marginBottom: form.type === "sell" ? "8px" : "0" }}>
                    {tradeLabel.text}
                  ...
                  {form.type === "sell" && previewProfit !== null && selectedSummary && (
                    <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span style={{ fontSize: "13px", color: COLORS.textSub }}>예상 실현수익</span>
                        <span style={{ fontSize: "13px", fontWeight: "700", color: previewProfit >= 0 ? COLORS.green : COLORS.red }}>{previewProfit >= 0 ? "+" : ""}{Math.round(previewProfit).toLocaleString()}{isUS ? "$" : "원"}</span>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span style={{ fontSize: "12px", color: COLORS.textSub }}>평단 기준</span>
                        <span style={{ fontSize: "12px", fontWeight: "600", color: previewProfit >= 0 ? COLORS.green : COLORS.red }}>{((Number(form.price) - selectedSummary.avgPrice) / selectedSummary.avgPrice * 100).toFixed(1)}%</span>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span style={{ fontSize: "12px", color: COLORS.textSub }}>원금 기준</span>
                        <span style={{ fontSize: "12px", fontWeight: "600", color: previewProfit >= 0 ? COLORS.green : COLORS.red }}>
                          {(previewProfit / selectedSummary.totalInvested * 100).toFixed(1)}%
                          <span style={{ fontWeight: "400", color: COLORS.textSub, marginLeft: "4px" }}>
                            {previewProfit >= 0
        ...
        {tab === "chart" && (
          <div>
            <div style={{ display: "flex", gap: "6px", marginBottom: "14px" }}>
              {["전체", "올해", "이번달"].map(p => (
                <button key={p} onClick={() => setChartPeriod(p)} style={{ padding: "7px 16px", borderRadius: "20px", border: "none", background: chartPeriod === p ? COLORS.blue : "white", color: chartPeriod === p ? "white" : COLORS.textSub, fontSize: "13px", fontWeight: chartPeriod === p ? "700" : "400", cursor: "pointer" }}>{p}</button>
              ))}
            </div>
            <div style={{ background: "white", borderRadius: "16px", padding: "20px", marginBottom: "12px" }}>
              ...
              <div style={{ fontSize: "26px", fontWeight: "700", color: chartTotalProfit >= 0 ? COLORS.green : COLORS.red }}>{chartTotalProfit >= 0 ? "+" : ""}{Math.round(chartTotalProfit).toLocaleString()}원</div>
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <BarChart data={monthlyData} margin={{ top: 4, right: 4, left: 0, bottom: 4 }}>
                      ...
                      <Bar dataKey="profit" radius={[6, 6, 0, 0]}>
                        {monthlyData.map((entry, i) => <Cell key={i} fill={entry.profit >= 0 ? COLORS.green : COLORS.red} />)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                )}
              ...
              {allSummaries.map(s => (
                <div key={s.name} style={{ marginBottom: "14px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
                    ...
                    <span style={{ fontSize: "13px", fontWeight: "700", color: s.realizedProfit >= 0 ? COLORS.green : COLORS.red }}>{s.realizedProfit >= 0 ? "+" : ""}{Math.round(s.realizedProfit).toLocaleString()}원</span>
                  </div>
                  <div style={{ height: "6px", background: COLORS.lightGray, borderRadius: "3px", overflow: "hidden" }}>
                    <div style={{ height: "100%", borderRadius: "3px", background: s.realizedProfit >= 0 ? COLORS.green : COLORS.red, width: `${Math.min(100, Math.abs(s.realizedProfit) / Math.max(...allSummaries.map(x => Math.abs(x.realizedProfit)), 1) * 100)}%`, transition: "width 0.4s" }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            <div style={{ background: "white", borderRadius: "16px", padding: "20px", marginBottom: "12px" }}>
              ...
              <div style={{ display: "flex", gap: "8px", marginBottom: "14px" }}>
                <button onClick={() => setNewStockMarket("KR")} style={{ flex: 1, padding: "10px", borderRadius: "10px", border: "none", background: newStockMarket === "KR" ? COLORS.blue : COLORS.lightGray, color: newStockMarket === "KR" ? "white" : COLORS.textSub, fontWeight: "700", fontSize: "13px", cursor: "pointer" }}>한국 주식</button>
                <button onClick={() => setNewStockMarket("US")} style={{ flex: 1, padding: "10px", borderRadius: "10px", border: "none", background: newStockMarket === "US" ? COLORS.blue : COLORS.lightGray, color: newStockMarket === "US" ? "white" : COLORS.textSub, fontWeight: "700", fontSize: "13px", cursor: "pointer" }}>미국 주식</button>
              </div>
              <button onClick={addStock} style={{ width: "100%", padding: "13px", borderRadius: "12px", border: "none", background: COLORS.blue, color: "white", fontSize: "14px", fontWeight: "700", cursor: "pointer" }}>종목 추가</button>
            </div>
              ...
              return (
                <div key={s.name} style={{ background: "white", borderRadius: "14px", padding: "16px", marginBottom: "10px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      <span style={{ fontWeight: "700", fontSize: "15px", color: COLORS.text }}>{s.name}</span>
                      <span style={{ fontSize: "11px", padding: "2px 7px", borderRadius: "20px", background: s.market === "KR" ? "#EBF3FE" : "#E6FAF6", color: s.market === "KR" ? COLORS.blue : COLORS.green, fontWeight: "600" }}>{s.market === "KR" ? "KR" : "US"}</span>
                    </div>
                  ...
                  <div style={{ display: "flex", gap: "10px", fontSize: "13px", color: COLORS.textSub }}>
                    ...
                    <span style={{ color: sum.realizedProfit >= 0 ? COLORS.green : COLORS.red, fontWeight: "600" }}>{sum.realizedProfit >= 0 ? "+" : ""}{Math.round(sum.realizedProfit).toLocaleString()}{s.market === "US" ? "$" : "원"}</span>
                  </div>
                </div>
              )
            })}
  ...
  {importResult && (
    <div style={{ marginTop: "10px", padding: "10px 12px", borderRadius: "8px", fontSize: "12px", background: importResult.success ? "#E6FAF6" : "#FEF0F1", color: importResult.success ? COLORS.green : COLORS.red }}>
                ...
                <div key={trade.id} style={{ background: "white", borderRadius: "14px", padding: "14px 16px", marginBottom: "8px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      ...
                      <span style={{ fontSize: "11px", padding: "2px 8px", borderRadius: "20px", background: trade.type === "buy" ? "#EBF3FE" : "#FEF0F1", color: trade.type === "buy" ? COLORS.blue : COLORS.red, fontWeight: "600" }}>
